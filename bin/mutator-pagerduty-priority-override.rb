    ...
    class PagerDuty
      class PriorityOverride
        ...
        def get_override(event, check, status)
          return nil if !event[:client].has_key?(:pd_override) || !event[:client][:pd_override].has_key?(check.to_sym)
          return event[:client][:pd_override][check.to_sym][status.to_sym]
        end
      end
    end
  end
